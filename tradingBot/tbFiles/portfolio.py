    ...
    def add_positions(self, positions: List[dict]) -> dict:
        ...
        if isinstance(positions, list):
            for position in positions:
                self.add_position(
                    symbol = position['symbol'], asset_type= position['asset_type'], purchase_price=position.get('purchase_price',0.0),
                    purchase_date= position.get('purchase_date',None), quantity=position.get('quantity',0)
                )
