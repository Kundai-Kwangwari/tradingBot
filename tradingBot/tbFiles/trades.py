...
class Trade():
    ...
    def new_trade(self, trade_id: str, order_type: str, side: str, enter_or_exit: str, price: float = 0.00,
        ...
        self.order_types = {'mkt' : 'MARKET', 'lmt': 'LIMIT','stop': 'STOP','stop_lmt': 'STOP_LIMIT',
                            'trailing_stop': 'TRAILING_STOP'}

        self.order_instructions = {'enter': {'long': 'BUY', 'short': ' SELL_SHORT'},
                                   'exit': {'long': 'SELL', 'short': 'BUY_TO_COVER'}}

        self.order = { "orderStrategyType": "SINGLE",
                       "orderType": self.order_types[order_type],
                       "session": "NORMAL",
                       "duration": "DAY",
                       "orderLegCollection": [{"instruction": self.order_instructions[enter_or_exit][side],
                                               "quantity": 0,
        ...
        if self.order['oderType'] == 'STOP':
            self.order['stopPrice'] = price

        elif self.order['oderType'] == 'LIMIT':
        ...
        elif self.order['oderType'] == 'TRAILING_STOP':
    ...
    def instrument(self, symbol:str, quantity: int, asset_type: str, sub_asset_type: str = None, order_leg_id: int = 0) -> dict:
        ...
        leg = self.order['oderLegCollection'][order_leg_id]
    ...
    def good_till_cancel(self, cancel_time: datetime) -> None:
        ...
        self.orfer['cancelTime'] = cancel_time.isoformat()
    ...
    def modify_side(self, side: Optional[str], order_leg_id: int=0) -> None:
        ...
        if side:
            ...
            self.order['orderLegCollection'][order_leg_id]['instructions'] = self.order_instructions[self.enter_or_exit]
            [self.side_opposite]
    ...
    def add_stop_loss(self, stop_size: float, percentage: bool = False) -> bool:
        ...
        if percentage:
            ...
            new_price = self.calculate_new_price(price = price, adjustment = adjustment, percentage = True)
        else:
            adjustment = -stop_size
            new_price = self.calculate_new_price(price = price, adjustment = adjustment, percentage = False)
    ...
    def add_stop_limit(self, stop_size: float, limit_size: float, stop_percentage: bool = False, limit_percentage: bool = False)-> bool :
        ...
        if stop_percentage:
            adjustment = 1.0 - stop_size
            stop_price = self.calculate_new_price(price = price, adjustment = adjustment, percentage = True)
        else:
            adjustment = -stop_size
            stop_price = self.calculate_new_price(price = price, adjustment = adjustment, percentage = False)

        if limit_percentage:
            adjustment = 1.0 - limit_size
            limit_price = self.calculate_new_price(price = price, adjustment = adjustment, percentage = True)
        else:
            adjustment = -limit_size
            limit_price = self.calculate_new_price(price = price, adjustment = adjustment, percentage = False)
    ...
    def add_take_profit(self, profit_size: float, percentage: bool = False) -> bool:
        ...
        if percentage:
            ...
            profit_price = self.calculate_new_price(price = price, adjustment = adjustment, percentage = True)
        else:
            adjustment = -profit_size
            profit_price = self.calculate_new_price(price = price, adjustment = adjustment, percentage = False)
    ...
    def modify_session(self, session :str) -> None:
        ...
        def _generate_order_id(self) -> str:
            ...
            if self.order:
                ...
                order_id = order_id.format(
                    symbol = self.symbol,
                    side = self.side,
                    enter_or_exit = self.enter_or_exit, timestamp = datetime.now().timestamp()
                )
            else:
                return ""
        ...
        def add_leg(self, order_leg_id: int, symbol: str, quantity: int, asset_type: str, sub_asset_type: str = None) -> List[Dict]:
            ...
            leg = {}
            leg['instrument']['symbol'] = symbol
            leg['isntrument']['assetType'] = asset_type
            leg['quantity'] = quantity
            ...
            if order_leg_id == 0:
                self.instrument(symbol = symbol, asset_type=asset_type,
                                quantity=quantity, sub_asset_type = sub_asset_type,
                                order_leg_id = 0
                )
