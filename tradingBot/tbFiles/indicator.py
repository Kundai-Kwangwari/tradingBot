...
from tbFiles.stockframe import StockFrame
...
class Indicators():
    ...
    def get_indicator_signals(self, indicator: Optional[str]) -> None:
    ...
    def rsi(self, period: int, method:str = 'wilders') -> pd.DataFrame:
        ...
        self._frame['down_day'] = self._price_groups['change_in_price'].transform(lambda x:np.where(x < 0, x.abs(), 0))
        ...
        relative_strength = self._frame['ewma_up'] / self._frame['emwa_down']
        ...
        self._frame.drop(lables = ['ewma_up', 'ewma_down', 'down_day', 'up_day','change_in_price'],
                         axis = 1, inplace = True)

        return self._frame

    def sam(self, period: int) -> pd.DataFrame:
    ...
    def refresh(self):
        ...
        self._price_groups = self._stockframe.symbol_groups
    ...
    def check_signals(self) -> Union[pd.DataFrame, None]:
        ...
        signals_df = self._stockframe._check_signals(indicators = self._indicator_signals)
        return signals_df
